import type { Element, Highlighter, MDCElement, MDCNode, MDCRoot, MDCText } from '@nuxtjs/mdc'
...
import { bundledLanguages as bundledLangs, bundledThemes, createJavaScriptRegexEngine } from 'shiki'
...
export function createTiptapToMDC(options?: TiptapToMDCMapOptions) {
  ...
  async function tiptapToMdc(node: JSONContent, options?: TiptapToMDCOptions): Promise<{ body: MDCRoot, data: Record<string, unknown> }> {
    // re-create slugs
    slugs = new Slugger()
  ...
  function tiptapNodeToMDC(node: JSONContent): MDCRoot | MDCNode | MDCNode[] {
  ...
  function createParagraphElement(node: JSONContent, propsArray: string[][], rest: object = {}): MDCElement {
  ...
  async function applyShikiSyntaxHighlighting(mdc: MDCRoot, theme: SyntaxHighlightTheme = { default: 'github-light', dark: 'github-dark' }) {
    ...
    visit(
      ...
      (n: unknown) => {
        Object.assign(n as MDCNode, { tag: (n as Element).tagName, props: (n as Element).properties, tagName: undefined, properties: undefined })
      },
    )
